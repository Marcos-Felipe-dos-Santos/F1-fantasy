...
2. Se a mudança toca lógica de simulação/balanceamento: **escreva primeiro o teste que falha** (baseline vermelho) capturando o comportamento pretendido. Rode e confirme que falha. Só então implemente até passar.
...
7. **NUNCA faça push nem abra PR remoto.** Deixe o commit local pronto e peça revisão do `senior-reviewer`. Aguarde aprovação explícita do dev pra qualquer coisa remota.

Se o plano estiver ambíguo ou você precisar tomar uma decisão de design, **pare e pergunte** — não improvise arquitetura. Prefira commits pequenos e mensagens no estilo convencional (`feat:`, `fix:`, `test:`).
